...
output_path <- get_opt('output_csv', 'data/shipley_checks/imputation/mixgb_cv_rmse_bill.csv')
nrounds_val <- as.integer(get_opt('nrounds', '300'))
device_val <- get_opt('device', 'cuda')
eta_val <- as.numeric(get_opt('eta', '0.3'))
folds_val <- as.integer(get_opt('folds', '10'))
...
# CANONICAL SLA: Use logSLA (sla_mm2_mg transformed) to match production pipeline
# Check if input has log-transformed or raw traits
log_target_vars <- c('logLA', 'logNmass', 'logLDMC', 'logSLA', 'logH', 'logSM')
raw_target_vars <- c('leaf_area_mm2', 'nmass_mg_g', 'ldmc_frac', 'sla_mm2_mg', 'plant_height_m', 'seed_mass_mg')

# Determine which trait set is present
if (all(log_target_vars %in% feature_cols_base)) {
  # Log-transformed traits (already transformed, no additional transform needed)
  trait_info_all <- tibble::tibble(
    trait = log_target_vars,
    transform = c('none', 'none', 'none', 'none', 'none', 'none')
  )
} else if (all(raw_target_vars %in% feature_cols_base)) {
  # Raw traits (need transformation)
  trait_info_all <- tibble::tibble(
    trait = raw_target_vars,
    transform = c('log', 'log', 'logit', 'log', 'log', 'log')
  )
} else {
  found_log <- intersect(log_target_vars, feature_cols_base)
  found_raw <- intersect(raw_target_vars, feature_cols_base)
  stop(sprintf('No complete trait set found. Log traits: %s, Raw traits: %s',
               paste(found_log, collapse=', '), paste(found_raw, collapse=', ')))
}
...
apply_transform <- function(x, type) {
  ...
  if (type == 'none') {
    return(x)  # Already transformed, return as-is
  }
  x
}
