...
VERIFICATION VERSION - Corrected FunGuild Counting
...
CORRECTION: Fixed funguild_genera counting to count DISTINCT genera (not row count)
...
- Rationale: Field is named "genera" (plural of genus), should count genera not guild assignments

Reference: Tanunchai et al. (2022) Microbial Ecology

Usage:
    python shipley_checks/src/Stage_4/python_sql_verification/01_extract_fungal_guilds_hybrid_VERIFIED.py
    python shipley_checks/src/Stage_4/python_sql_verification/01_extract_fungal_guilds_hybrid_VERIFIED.py --test --limit 100
"""
...
def extract_hybrid_guilds(limit=None):
    ...
    print("="*80)
    print("STAGE 4.1: Fungal Guild Extraction - VERIFIED VERSION")
    print("="*80)
    ...
    print("  - VERIFIED: COUNT DISTINCT genera (not row count)")
    print()
    ...
    # Save
    output_file = output_dir / ('plant_fungal_guilds_hybrid_11711_VERIFIED_test.parquet' if limit else 'plant_fungal_guilds_hybrid_11711_VERIFIED.parquet')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Extract fungal guild profiles using hybrid approach (DuckDB-optimized, VERIFIED)')
