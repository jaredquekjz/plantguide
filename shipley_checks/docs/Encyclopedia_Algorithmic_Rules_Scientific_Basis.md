# Encyclopedia Algorithmic Rules: Scientific Basis and Horticultural Translation

**Document Purpose**: Detailed scientific justification for translating ecological trait data into actionable horticultural recommendations.
...
### Overview and Guide
1. [Scientific Basis: Two Complementary Occurrence-Based Perspectives](#1-scientific-basis-two-complementary-occurrence-based-perspectives)
2. [Encyclopedia Article Layout Guide](#2-encyclopedia-article-layout-guide)

### Part I: Environmental Envelope from Occurrence Data
3. [Climate Envelope (WorldClim)](#3-climate-envelope-from-natural-occurrence-data)
4. [Soil Envelope (SoilGrids)](#4-soil-envelope-from-natural-occurrence-data)

### Part II: Ecological Indicator Values (EIVE)
5. [EIVE Scientific Foundation and Methodology](#5-eive-scientific-foundation-and-methodology)
6. [EIVE Indicators to Growing Conditions](#6-eive-indicators-to-growing-conditions)

### Part III: Functional Traits and Derived Properties
7. [CSR Strategy to Maintenance Requirements](#7-csr-strategy-to-maintenance-requirements)
8. [Ecosystem Services Quantification](#8-ecosystem-services-quantification)
9. [Biological Interactions to Pest/Disease Risk](#9-biological-interactions-to-pestdisease-risk)

### Part IV: Implementation Reference
10. [Implementation Thresholds Summary](#10-implementation-thresholds-summary)
11. [References](#11-references)

---

## 1. Scientific Basis: Two Complementary Occurrence-Based Perspectives

This encyclopedia integrates two complementary data sources that both describe **where plants naturally occur**, but through different methodologies. Understanding both perspectives enables gardeners to triangulate recommendations with greater confidence.

### 1.1 Perspective A: Raw Environmental Envelope (WorldClim/SoilGrids)

**Method**: Direct extraction of environmental variables at GBIF occurrence coordinates
- Source: ~145 million georeferenced occurrence records from herbarium specimens, field surveys, and biodiversity databases (GBIF)
- Processing: For each occurrence point, extract WorldClim climate and SoilGrids soil values, then aggregate across all occurrences per species
- Output: q05/q50/q95 percentiles describing the environmental envelope

**Interpretation**: Describes the **fundamental environmental conditions** where the plant is found. The q50 shows typical conditions; q05/q95 show tolerance limits (extremes where populations exist).

**Strengths**: Objective, quantitative, directly measurable environmental values (actual temperatures in °C, precipitation in mm, soil pH values)

### 1.2 Perspective B: Ecological Indicator Values for Europe (EIVE)

**Method**: Expert synthesis of 31 regional phytosociological indicator value systems
- Source: Field observations of plant occurrence within vegetation communities across Europe (Ellenberg tradition, 1950s-present)
- Processing: Harmonised and rescaled to 0-10 continuous scale by Dengler et al. (2022)
- Output: Niche position (central tendency) and niche width (tolerance breadth) for 5 dimensions

**Interpretation**: Describes the **realised ecological niche** - where the plant occurs under competition and facilitation with co-occurring species. This captures competitive optima, not just survival limits.

**Strengths**: Integrates biotic interactions; reflects where plants actually thrive in communities; validated against occurrence data (r=0.859 for temperature)

### 1.3 Complementary Use

| Question | Use This Source |
|----------|-----------------|
| "Will this plant survive in my conditions?" | Environmental envelope (q05/q95 tolerance limits) |
| "Will this plant thrive and compete well?" | EIVE niche position (competitive optimum) |
| "What are the actual temperature/rainfall values?" | Environmental envelope (quantitative) |
| "Is this a shade-lover or sun-seeker?" | EIVE-L (relative light preference) |
| "How much should I water?" | EIVE-M + precipitation envelope together |

When both sources agree (e.g., EIVE-T indicates warmth preference AND climate envelope shows Mediterranean distribution), confidence is high. When they diverge, the plant may survive broadly but perform best in specific conditions.

### 1.4 Our Validation

Our Stage 2 XGBoost modelling independently confirms the strong relationship between EIVE indicators and environmental occurrence data:

| EIVE Axis | R² (traits → EIVE) | Accuracy ±1 class | Interpretation |
|-----------|-------------------|-------------------|----------------|
| T (Temperature) | 0.806 | 93.5% | Excellent - strong climate signal |
| M (Moisture) | 0.661 | 89.3% | Good - precipitation + soil drainage |
| L (Light) | 0.587 | 87.3% | Good - habitat/canopy structure |
| N (Nitrogen) | 0.610 | 80.7% | Moderate - soil fertility complex |
| R (Reaction/pH) | 0.437 | 81.2% | Moderate - soil chemistry varies locally |

This confirms EIVE indicators can be reliably predicted from environmental and trait data, validating their ecological basis.

---

## 2. Encyclopedia Article Layout Guide

Each plant species article in the encyclopedia follows a standardised structure that presents information in order of importance for gardening decisions:

### Article Structure

**1. Header Section**
- Common name(s) and accepted scientific name (WFO-verified)
- Family and taxonomic classification
- Quick-reference tags:
  - Life form/growth form (tree, shrub, perennial, annual, etc.)
  - Typical height range (from trait data)
  - Hardiness zone (from TNn_q05)
  - Native range/distribution (from occurrence data)
  - Köppen climate zones (natural occurrence)


**2. Growing Conditions** (Integrated Environmental + EIVE Perspectives)

*Present both occurrence-based perspectives together for triangulation (Section 1)*

- **Light Requirements**: EIVE-L (Section 6)
  - EIVE-L indicator: 0-10 continuous scale (deep shade → full sun)
  - No direct environmental envelope equivalent; EIVE-L is primary source
  - Use for siting decisions: shade garden, woodland edge, open border, full sun

- **Climate Conditions**: Environmental envelope (Section 3) + EIVE-T, EIVE-M (Section 6)

  *Köppen Climate Classification:*
  - Natural occurrence zones (from Stage 4 classification): e.g., "Cfb (Temperate oceanic), Csa (Mediterranean)"
  - Interpretation for gardeners: "Thrives in temperate maritime climates; tolerates Mediterranean conditions"

  *Temperature:*
  - Environmental envelope: TNn_q05 (absolute cold hardiness), TXx_q95 (heat tolerance), BIO_5/BIO_6 q05/q50/q95, BIO_4 (seasonality/continentality)
  - EIVE-T indicator: 0-10 continuous scale (arctic-alpine → subtropical)
  - Practical translation: USDA hardiness zone, heat zone, seasonal adaptation
  - Triangulation: Compare EIVE-T position with actual temperature ranges for confidence

  *Moisture/Precipitation:*
  - Environmental envelope: BIO_12/BIO_14 q05/q50/q95 (annual/driest month precipitation), BIO_17/BIO_18/BIO_19 (seasonal), CDD_q95 (drought spell length)
  - EIVE-M indicator: 0-10 continuous scale (extreme drought → aquatic)
  - Practical translation: Watering frequency, drought tolerance rating, wet/dry season adaptation
  - Triangulation: EIVE-M shows competitive optimum; envelope shows survival range

- **Soil Conditions**: Environmental envelope (Section 4) + EIVE-R, EIVE-N (Section 6)

  *pH/Reaction:*
  - Environmental envelope: phh2o q05/q50/q95 at 0-5cm depth (actual pH values at occurrence sites)
  - EIVE-R indicator: 0-10 continuous scale (strongly acidic → calcareous/alkaline)
  - Practical translation: Compost type (ericaceous/standard/lime-tolerant), lime requirement
  - Triangulation: Envelope gives tolerance range; EIVE-R shows where plant thrives competitively

  *Nitrogen/Fertility:*
  - Environmental envelope: Soil nitrogen q05/q50/q95 (g/kg), CEC q05/q50/q95 (cmol/kg) at 0-5cm depth
  - EIVE-N indicator: 0-10 continuous scale (oligotrophic infertile → manure-rich)
  - Practical translation: Feeding schedule, fertilizer type/rate
  - Triangulation: Both inform feeding requirements; EIVE-N reflects competitive strategy

  *Texture and Structure:*
  - Environmental envelope: Clay/sand q05/q50/q95 (%), bulk density q05/q50/q95 (g/cm³), SOC q05/q50/q95 (g/kg) at 0-5cm depth
  - No direct EIVE equivalent (captured partly in EIVE-M for drainage)
  - Practical translation: Soil preparation, drainage needs, compost quantity
  - Use envelope data for compost selection and soil preparation

**3.  Cultivation Strategies** 

- **Propagation**:
  - Seed mass: Propagation strategy, germination expectations

- **CSR Strategy**: Competitor/Stress-tolerator/Ruderal classification (Section 7)
  - C/S/R percentages and interpretation
  - Growth rate and vigour expectations
  - Maintenance requirements (pruning/feeding/mulching frequency)
  - Spacing and management recommendations
  - Competitive ability and invasive potential

**4. Garden Value** 

- **Ecosystem Services**: Pre-calculated categorical ratings (Section 8)
  - Carbon storage (biomass, recalcitrant, total): Rating + confidence level
  - Nitrogen fixation: Rating + confidence level (based on TRY database for legumes)
  - Erosion protection: Rating + confidence level
  - NPP (Net Primary Productivity): Rating + confidence level
  - Decomposition rate: Rating + confidence level
  - Nutrient cycling/retention/loss: Ratings + confidence levels
  - All ratings: Very High / High / Moderate / Low / Very Low (from CSR-derived framework)

- **Pollinator Support**: From GloBI interaction data (Section 9)
  - Documented pollinator species and diversity
  - Coverage metric (% of plants with documented pollinators)
  - Shared pollinator networks within guild

- **Pest and Disease Considerations**: From GloBI biological interaction networks (Section 9)
  - Known herbivore associations and pest pressure
  - Known pathogen associations and disease risk
  - Natural enemy associations (predators, parasitoids, entomopathogens - and specific pests they target (if available)
  - Management implications

### Information Hierarchy

**Primary sources** (always present):
1. Environmental envelope (climate + soil from occurrence data)
2. EIVE indicators (phytosociological niche)
3. CSR strategy (maintenance requirements)

**Secondary sources** (when available):
4. Ecosystem services (calculated from traits + literature)
5. Biological interactions (derived from trait-based food web models)

**Tertiary sources** (supplementary):
6. Horticultural observations (RHS, literature)
7. Cultivar information (trade sources, trials)
...
## 7. CSR Strategy to Maintenance Requirements

### 7.1 Scientific Foundation

**Theory**: Grime's CSR (Competitor-Stress tolerator-Ruderal) plant strategy theory classifies plants by their adaptation to:
- **Competition (C)**: Resource-rich, undisturbed habitats → fast growth, high nutrient demand
- **Stress (S)**: Resource-poor or extreme habitats → slow growth, conservative physiology
- **Disturbance (R)**: Frequently disturbed habitats → rapid reproduction, short lifespan

**Data Source**: Pierce et al. (2017) StrateFy global calibration using leaf traits (LA, LDMC, SLA)

**Reference**: Pierce S, Negreiros D, Cerabolini BEL, et al. (2017) A global method for calculating plant CSR ecological strategies applied across biomes worldwide. *Functional Ecology* 31: 444-457.

### 7.2 Strategy to Horticultural Traits

| Property | C-Dominant (C>60%) | S-Dominant (S>60%) | R-Dominant (R>60%) |
|----------|-------------------|-------------------|-------------------|
...
| **Maintenance** | HIGH (5-7 hrs/yr) | LOW (1-2 hrs/yr) | MEDIUM (replanting needed) |
| **Pruning** | Regular shaping | Minimal | Deadheading to prevent spread |
| **Lifespan** | Variable | Often long-lived | Short (annual-biennial) |
| **Decomposition** | Variable | Slow (recalcitrant) | Fast (labile) |

### 7.3 Maintenance Level Algorithm

```rust
/// Calculate maintenance level from CSR scores
/// Based on Pierce et al. (2017) thresholds and horticultural experience
fn calculate_maintenance_level(c: f64, s: f64, r: f64) -> MaintenanceLevel {
    // Normalize to percentages if needed
    let (c, s, r) = if c + s + r > 1.5 {
        (c / 100.0, s / 100.0, r / 100.0)
    } else {
        (c, s, r)
    };

    // Primary strategy dominance
    if s > 0.60 {
        MaintenanceLevel::Low  // Stress-tolerators: minimal care
    } else if s > 0.50 {
        MaintenanceLevel::LowMedium
    } else if c > 0.60 {
        MaintenanceLevel::High  // Competitors: regular pruning, feeding
    } else if c > 0.50 {
        MaintenanceLevel::MediumHigh
    } else if r > 0.60 {
        MaintenanceLevel::Medium  // Ruderals: replanting, deadheading
    } else {
        MaintenanceLevel::Medium  // Balanced CSR
    }
}

/// Time estimates per maintenance level (hours per plant per year)
fn get_time_estimate(level: MaintenanceLevel, height_m: f64) -> f64 {
    let base = match level {
        MaintenanceLevel::Low => 1.0,
        MaintenanceLevel::LowMedium => 2.0,
        MaintenanceLevel::Medium => 3.0,
        MaintenanceLevel::MediumHigh => 4.5,
        MaintenanceLevel::High => 6.0,
    };

    // Scale by plant size (larger plants = more work)
    let size_factor = (height_m / 2.0).max(0.5).min(2.0);
    base * size_factor
}
```

### 7.4 Growth Rate Advice

```rust
fn get_growth_rate_advice(c: f64, s: f64, r: f64) -> String {
    let dominant = if c > s && c > r { "C" }
                   else if s > c && s > r { "S" }
                   else { "R" };

    match dominant {
        "C" => "🌿 **Growth Rate**: Fast (C strategy)
   → Vigorous grower; benefits from rich soil
   → Regular pruning to maintain shape
   → May outcompete neighbors; give adequate space".into(),
        "S" => "🌿 **Growth Rate**: Slow (S strategy)
   → Steady, conservative growth
   → Minimal pruning needed
   → Drought-tolerant once established; avoid overwatering".into(),
        "R" => "🌿 **Growth Rate**: Rapid establishment (R strategy)
   → Quick to establish from seed
   → May self-seed prolifically; deadhead to control spread
   → Short-lived; plan for succession".into(),
        _ => "🌿 **Growth Rate**: Moderate (balanced CSR)".into(),
    }
}
```
...
**Implementation Note**: Pre-computed semantic bins from `lookup_tables.rs` provide scientifically-derived class boundaries and narrative labels that should be used verbatim in encyclopedia output.


## 9. Biological Interactions to Pest/Disease Risk

### 9.1 Pest Pressure Assessment

**Algorithm**:

```rust
fn assess_pest_pressure(
    herbivore_count: usize,
    predator_count: usize,
    entomopathogen_count: usize,
) -> PestAssessment {
    let natural_enemies = predator_count + entomopathogen_count;
    let control_ratio = if herbivore_count > 0 {
        natural_enemies as f64 / herbivore_count as f64
    } else {
        1.0
    };

    let (risk_level, advice) = match (herbivore_count, control_ratio) {
        (0..=5, _) => ("LOW", "Minimal pest pressure expected"),
        (6..=15, r) if r > 0.3 => ("LOW-MODERATE", "Some pests but good natural control"),
        (6..=15, _) => ("MODERATE", "Moderate pest pressure; monitor regularly"),
        (16..=30, r) if r > 0.2 => ("MODERATE", "Consider companion planting for biocontrol"),
        (16..=30, _) => ("MODERATE-HIGH", "Regular monitoring recommended; organic controls may be needed"),
        (_, r) if r > 0.15 => ("HIGH", "High pest pressure but some natural control available"),
        _ => ("HIGH", "High pest pressure; integrated pest management recommended"),
    };

    PestAssessment {
        herbivore_count,
        natural_enemy_count: natural_enemies,
        control_ratio,
...
### 9.2 Disease Risk Assessment

**Algorithm**:

```rust
fn assess_disease_risk(
    pathogen_count: usize,
    antagonist_count: usize,  // Mycoparasites, Trichoderma, etc.
) -> DiseaseAssessment {
    let control_ratio = if pathogen_count > 0 {
        antagonist_count as f64 / pathogen_count as f64
    } else {
        1.0
    };

    let (risk_level, advice) = match (pathogen_count, control_ratio) {
        (0..=5, _) => ("LOW", "Few known disease issues"),
        (6..=20, r) if r > 0.2 => ("LOW-MODERATE", "Some diseases but antagonistic fungi help"),
        (6..=20, _) => ("MODERATE", "Monitor for disease; ensure good air circulation"),
        (21..=50, _) => ("MODERATE-HIGH", "Disease-prone; preventive measures recommended"),
        _ => ("HIGH", "High disease susceptibility; consider resistant cultivars"),
    };
...
## 10. Implementation Thresholds Summary

### 10.1 Climate Tolerance (from Natural Occurrence Range)

**Cold Hardiness** (use TNn_q05 for absolute minimum, or BIO_6_q05 for monthly minimum):

| TNn q05 (°C) | Hardiness Level | Gardening Advice |
|--------------|-----------------|------------------|
| < -40 | Extremely hardy | Survives severe continental winters |
| -40 to -25 | Very hardy | Reliable in cold temperate climates |
| -25 to -15 | Cold-hardy | Survives hard frosts; most temperate gardens |
| -15 to -5 | Moderately hardy | Survives moderate frosts; mulch recommended |
| -5 to 0 | Half-hardy | Light frost only; protect roots |
| > 0 | Frost-tender | Requires frost protection |

**Heat Tolerance** (use TXx_q95 for absolute maximum, or BIO_5_q95 for monthly maximum):

| TXx q95 (°C) | Heat Tolerance | Gardening Advice |
|--------------|----------------|------------------|
...
| 30-35 | Moderate | Prefers moderate summers; shade in extreme heat |
| < 30 | Cool-climate | Struggles in hot summers; shade essential |

**Drought Tolerance** (use BIO_14_q05 for driest month, CDD_q95 for longest dry spell):

| BIO_14 q05 (mm) | CDD q95 (days) | Drought Class | Watering Advice |
|-----------------|----------------|---------------|-----------------|
| < 5 | > 60 | Extreme | Water sparingly; overwatering harmful |
| 5-15 | 30-60 | Strong | Deep infrequent watering once established |
| 15-30 | 14-30 | Moderate | Water during dry spells |
| 30-50 | < 14 | Limited | Regular watering in summer |
| > 50 | < 7 | None | Don't allow to dry out |

**Seasonality** (use BIO_4 for temperature seasonality):

| BIO_4 q50 | Climate Type | Advice |
|-----------|--------------|--------|
| < 200 | Oceanic/tropical | May struggle with extreme seasons |
| 200-400 | Maritime temperate | Suits most coastal/mild gardens |
...
**Heat Stress Indicators** (use SU for summer days, TR for tropical nights):

| SU q50 (days >25°C) | Heat Zone | Night Cooling (TR q50) |
|---------------------|-----------|------------------------|
| < 7 | Cool summers | < 5: Requires cool nights |
| 7-30 | Mild summers | 5-30: Tolerates occasional warm nights |
| 30-60 | Warm summers | 30-90: Adapted to warm nights |
| 60-120 | Hot summers | > 90: Thrives with tropical nights |
| > 120 | Very hot summers | - |

**Severe Winter Indicators** (use ID for icing days, CSDI for cold spells):

| ID q50 (days Tmax<0°C) | Winter Severity |
|------------------------|-----------------|
| < 1 | Virtually frost-free |
| 1-10 | Occasional severe cold |
| 10-30 | Regular severe cold |
| 30-60 | Extended severe winters |
| > 60 | Prolonged severe winters |

### 10.2 CSR Maintenance

| CSR Dominance | Threshold | Maintenance Level |
|---------------|-----------|-------------------|
| S > 60% | Pure stress-tolerator | LOW (1-2 hrs/yr) |
| S > 50% | S-leaning | LOW-MEDIUM |
| Balanced | No dominance | MEDIUM (3-4 hrs/yr) |
| C > 50% | C-leaning | MEDIUM-HIGH |
| C > 60% | Pure competitor | HIGH (5-7 hrs/yr) |

### 10.3 EIVE Indicators (Continuous 0-10 Scale)

| Axis | Scale | Low End (0-2) | Mid Range (4-6) | High End (8-10) |
|------|-------|---------------|-----------------|-----------------|
| L (Light) | 0-10 | Deep shade (<1% illumination) | Partial shade/semi-shade | Full sun required |
| M (Moisture) | 0-10 | Extreme drought tolerance | Moist, well-watered | Aquatic/waterlogged |
| T (Temperature) | 0-10 | Arctic-alpine | Cool-temperate | Subtropical/warm |
| R (Reaction) | 0-10 | Strongly acidic (calcifuge) | Slightly acidic to neutral | Calcareous/alkaline (calcicole) |
| N (Nitrogen) | 0-10 | Oligotrophic (infertile) | Mesotrophic (moderate) | Eutrophic (manure-rich) |

**Note**: All EIVE indicators use a harmonized 0-10 continuous scale (Dengler et al. 2022). Values are continuous, not categorical classes.

### 10.4 Ecosystem Services

| Service | Rating 1-2 | Rating 3 | Rating 4-5 |
|---------|------------|----------|------------|
| Carbon | <5 kg CO₂/yr | 5-15 kg | >15 kg |
| N-fixation | 0 | 50-100 kg N/ha | >150 kg N/ha |
| Erosion | Poor | Moderate | Excellent |
| Pollination | <5 species | 5-20 species | >20 species |

### 10.5 Soil Envelope (from Natural Occurrence Range)

| Property | Data Source | Interpretation |
|----------|-------------|----------------|
| pH | sg_phh2o_q05/q50/q95 | Range where plant naturally occurs |
| Clay % | sg_clay_q05/q50/q95 | Texture tolerance from occurrences |
| Sand % | sg_sand_q05/q50/q95 | Drainage preference from occurrences |
| SOC (g/kg) | sg_soc_q05/q50/q95 | Organic matter preference |
| CEC (cmol/kg) | sg_cec_q05/q50/q95 | Fertility level tolerance |
| BDOD (cg/cm³) | sg_bdod_q05/q50/q95 | Soil structure preference |

**Interpretation Key**:
- **q05**: 5th percentile = low end of natural range
- **q50**: Median = typical conditions where plant thrives
- **q95**: 95th percentile = high end of natural range
- Wide q05-q95 range = broad tolerance; narrow range = specialist

---

## 11. References

1. Pierce S, et al. (2017) A global method for calculating plant CSR ecological strategies. *Functional Ecology* 31: 444-457.

2. Dengler J, et al. (2023) EIVE 1.0 - Ellenberg-type indicator values for European vascular plants. *Vegetation Classification and Survey* 4.

3. USDA Plant Hardiness Zone Map (2023). https://planthardiness.ars.usda.gov/

4. American Horticultural Society Heat Zone Map. https://ahsgardening.org/gardening-resources/gardening-maps/heat-zone-map/

5. EPA Carbon Sequestration Calculation Methods. https://www.epa.gov/

6. SoilGrids 2.0: Global soil information. https://soilgrids.org/

7. WorldClim 2.1 Bioclimatic Variables. https://www.worldclim.org/

---

*Document generated for Ellenberg Plant Encyclopedia project, 2025*
