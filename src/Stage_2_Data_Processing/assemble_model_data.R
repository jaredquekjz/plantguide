...
# accepted names) with EIVE indicator values. Produces two CSVs:
#  - artifacts/model_data_full.csv
#  - artifacts/model_data_complete_case.csv (six traits present; SSD uses 'combined')
#
# Key behaviors
#  - Join key: lowercased, trimmed species names; TRY column
#    'Species name standardized against TPL' matched to EIVE 'wfo_accepted_name'.
#  - SSD handling: Provide 'SSD used (mg/mm3)' which prefers observed over
#    combined when available; also add 'ssd_imputed_used' flag (1 if used value
#    came from imputation via LDMC; 0 if observed; NA if missing).
#  - Provenance: Carry per-trait record counts ('(n.o.)' columns) and compute a
#    helper 'min_records_6traits' across the six traits.
...
# Outputs
out_full       <- opts[["out_full_csv"]]      %||% "artifacts/model_data_full.csv"
out_complete   <- opts[["out_complete_csv"]]  %||% "artifacts/model_data_complete_case.csv"
emit_obs_ssd   <- tolower(opts[["emit_observed_ssd_complete"]] %||% "false") %in% c("1","true","yes","y")
out_complete_o <- opts[["out_complete_observed_ssd_csv"]] %||% "artifacts/model_data_complete_case_observed_ssd.csv"
...
if (have_readr) {
  readr::write_csv(model_full[, setdiff(names(model_full), ".__species_norm"), drop = FALSE], out_full)
  readr::write_csv(model_cc[,   setdiff(names(model_cc),   ".__species_norm"), drop = FALSE], out_complete)
  if (!is.null(model_cc_obs)) {
    ensure_dir(out_complete_o)
    readr::write_csv(model_cc_obs[, setdiff(names(model_cc_obs), ".__species_norm"), drop = FALSE], out_complete_o)
  }
} else {
  utils::write.csv(model_full[, setdiff(names(model_full), ".__species_norm"), drop = FALSE], out_full, row.names = FALSE)
  utils::write.csv(model_cc[,   setdiff(names(model_cc),   ".__species_norm"), drop = FALSE], out_complete, row.names = FALSE)
  if (!is.null(model_cc_obs)) {
    ensure_dir(out_complete_o)
    utils::write.csv(model_cc_obs[, setdiff(names(model_cc_obs), ".__species_norm"), drop = FALSE], out_complete_o, row.names = FALSE)
  }
}
