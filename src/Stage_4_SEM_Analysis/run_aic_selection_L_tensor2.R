...
dataset <- "artifacts/model_data_bioclim_subset_sem_ready_20250920_stage2_pcs.csv"
...
if ("EIVEres-M" %in% names(data) && !("EIVEres_M" %in% names(data))) {
...
formula_txt <- paste(resp, "~",
  ...
  "+ s(lma_precip, k=5) + te(pc_trait_2, precip_mean, k=c(5,5)) + te(pc_trait_1, mat_mean, k=c(5,5))")

form <- as.formula(formula_txt)
cat("Fitting GAM with two tensor surfaces...\n")
...
out_dir <- "results/aic_selection_L_tensor2"
