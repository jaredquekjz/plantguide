...
if (any(!pc_cols %in% names(data))) stop("Missing trait PCs")
...
formula_txt <- paste(resp, "~",
  ...
  "+ s(lma_precip, k=5) + te(pc_trait_2, precip_mean, k=c(5,5)) + te(pc_trait_1, mat_mean, k=c(5,5)) + te(pc_trait_3, temp_seasonality, k=c(5,5))")

form <- as.formula(formula_txt)
cat("Fitting GAM with three tensor surfaces...\n")
...
out_dir <- "results/aic_selection_L_tensor3"
