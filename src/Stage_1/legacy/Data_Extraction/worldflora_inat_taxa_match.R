...
cat("Starting WorldFlora matching for iNaturalist taxa\n")
flush.console()
...
input_path <- file.path(repo_root, "data/external/inat/manifests/inat_taxa_names_for_r.csv")
output_path <- file.path(repo_root, "data/external/inat/manifests/inat_taxa_wfo_worldflora.csv")
wfo_path <- file.path(repo_root, "data/classification.csv")

log_msg("Reading iNat taxa names from: ", input_path)
inat <- fread(input_path, encoding = "UTF-8", data.table = FALSE)
log_msg("Loaded ", nrow(inat), " taxon rows")

inat$name_raw <- trimws(inat$name)
inat <- inat[!is.na(inat$name_raw) & nchar(inat$name_raw) > 0, ]
log_msg("Retained ", nrow(inat), " rows with non-empty names")
