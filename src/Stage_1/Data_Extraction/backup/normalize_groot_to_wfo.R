#!/usr/bin/env Rscript
# Normalize GROOT species names to WFO taxonomy
# GROOT uses TNRS (Taxonomic Name Resolution Service) naming
# We map these to WFO accepted names using the same approach as EIVE normalization
...
out_csv  <- trimws(gsub('[\r\n`]+','', get_arg('--out', 'data/GROOT/GROOT_species_WFO.csv')))
...
cat("=== GROOT to WFO Normalization Script ===\n")
...
if (!is.null(WFO.data)) {
  ...
  message(sprintf('Exact normalized matches (WFO CSV): %d of %d', exact_n, length(names_in)))
} else {
...
if (!is.null(WFO.data)) {
  ...
  if (nrow(exact_matched) > 0) {
    fwrite(exact_matched[, .(scientific_name, wfo_id, wfo_accepted_name)], out_csv)
...
message(sprintf('Parameters: groot_csv=%s; wfo_csv=%s; out=%s; fuzzy=%d; batch_size=%d; checkpoint_every=%d',
                groot_csv, ifelse(nzchar(wfo_csv), wfo_csv, 'NONE'), out_csv, fuzzy, batch_size, checkpoint_every))
...
if (fuzzy <= 0) {
  ...
  if (nrow(unmatched) > 0) {
    ...
    fwrite(unmatched[, .(scientific_name, wfo_id = NA_character_, wfo_accepted_name = NA_character_)], 
  ...
  todo <- res[is.na(wfo_id), unique(scientific_name)]
  if (length(todo)) {
    ...
    if (wf_source == 'none') {
      ...
      for (start in seq.int(1L, total, by = batch_size)) {
        ...
        if (!is.null(wf_res)) {
          ...
          matched[, scientific_name := get(in_col)]
          matched[, norm := normalize_name(scientific_name)]
          ...
          fuzzy_batch <- matched[!is.na(wfo_id), .(scientific_name, wfo_id, wfo_accepted_name = if ('wfo_accepted_name' %in% names(matched)) wfo_accepted_name else NA_character_)]
