#!/usr/bin/env python3
"""
Step 3: Update 1,084 Modelling Dataset with Full Quantiles
==========================================================
...
Input:
- model_data/inputs/modelling_master_1084_20251029.parquet (1,084 × 273)
- model_data/inputs/env_features_full_quantiles_11680_20251029.parquet (11,680 × 625)

Output:
- model_data/inputs/modelling_master_1084_20251029.parquet (1,084 × 741)
  (273 - 156 + 624 = 741)
...
def main():
    print("="*60)
    print("Step 3: Updating 1,084 Modelling Dataset")
    print("="*60)

    # Load current modelling dataset
    print("\n1. Loading current modelling dataset...")
    master_path = Path("model_data/inputs/modelling_master_1084_20251029.parquet")
    ...
    # Load full quantiles (11,680 species)
    ...
    quantiles_all = pd.read_parquet(quantiles_path)
    print(f"   Full quantiles: {quantiles_all.shape[0]:,} × {quantiles_all.shape[1]}")

    # Filter quantiles to 1,084 species
    quantiles = quantiles_all[quantiles_all['wfo_taxon_id'].isin(master_base['wfo_taxon_id'])].copy()
    print(f"   Filtered to 1,084 species: {quantiles.shape[0]:,} × {quantiles.shape[1]}")
    ...
    assert updated.shape == (1084, expected_cols), \
        f"Expected 1,084 × {expected_cols}, got {updated.shape}"
