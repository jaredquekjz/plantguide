...
def build_multitrophic_network(test_mode=False):
    ...
    print("="*80)
    print("STAGE 4.2: Build Multi-Trophic Network")
    print("="*80)
    ...
    profile_file = 'plant_organism_profiles_test.parquet' if test_mode else 'plant_organism_profiles.parquet'
    ...
    # Step 5: Save results
    output_predators = output_dir / ('herbivore_predators_test.parquet' if test_mode else 'herbivore_predators.parquet')
    output_antagonists = output_dir / ('pathogen_antagonists_test.parquet' if test_mode else 'pathogen_antagonists.parquet')
