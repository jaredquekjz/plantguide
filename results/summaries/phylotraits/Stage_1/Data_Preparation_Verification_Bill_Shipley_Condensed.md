...
1. **Code logic review**: Examine R scripts for methodological soundness:
   ...
   - SLA waterfall logic and anti-leakage measures

2. **Ecological sanity checks**: Verify biological plausibility at key checkpoints:
   - EIVE ranges (L/T/M/R: 1-9, N: 1-12)
   - TRY trait ranges (height 0.01-100m, LMA 20-500 g/m²)
   - Taxonomic distributions (common genera present)
   - Coverage percentages (match rates ~84-99%)
...
**Phase 3**: Extract phylogeny → Assemble final dataset

---

## Required Source Data (Read-Only)

Bill's scripts read from these **13 canonical files**. All outputs write to `data/shipley_checks/`.
...
```bash
# Run 8 WorldFlora scripts (~3-4 hours total)
...
**Prerequisites**: Phylogenetic tree for 11,711 species built (see Phase 3 overview in XGBoost doc)
...
- Environmental: `data/shipley_checks/*_species_{summary,quantiles}_R.parquet`
- Phylogeny: `data/shipley_checks/modelling/phylo_eigenvectors_11711_bill.csv`
- Final dataset: `data/shipley_checks/modelling/canonical_imputation_input_11711_bill.csv`
...
install.packages(c("arrow", "dplyr", "data.table", "WorldFlora", "ape"))
```
