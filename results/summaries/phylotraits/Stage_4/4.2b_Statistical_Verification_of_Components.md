...
For each pair (Plant A, Plant B):
  For each pathogen p of Plant A:
    specific_matches = |antagonists(p) ∩ mycoparasites(B)| × 1.0  (GloBI - rarely fires)
    general_myco = |mycoparasites(B)| × 1.0  (FungalTraits - PRIMARY mechanism)

P2_raw = Σ(specific_matches + general_myco) / max_pairs × 10
P2_norm = tanh(P2_raw)
```

**Weight updated**: General mycoparasites now weighted at 1.0 (was 0.3) because it's the ONLY reliable mechanism.
...
**GloBI pathogen-antagonist data has INVERTED relationship direction**:
- `pathogen_antagonists.parquet` column names are misleading:
  - **"pathogen" column = VICTIM** (the thing being attacked)
  - **"antagonists" column = ATTACKERS** (the things doing the attacking)
- Example 1: `pathogen: Poaceae → antagonists: [herbivores, grass parasites]`
  - Translation: "Grass is attacked by herbivores" (plant → herbivores)
  - NOT what we need: "Fungal pathogen is attacked by mycoparasites" (fungi → fungi)
- Example 2: `pathogen: Solanum lycopersicum → antagonists: [Phytophthora, Fusarium, Botrytis]`
  - Translation: "Tomato is attacked by fungal pathogens" (plant → fungi)
  - Still not fungi → fungi relationships
- Example 3: `pathogen: Botrytis cinerea → antagonists: [Botrytis cinerea hypovirus 4, ...]`
  - Translation: "Botrytis is attacked by viruses" (fungi → viruses)
  - Close, but viruses aren't mycoparasite fungi

**Why the data is unusable for P2**:
- **~80% of rows**: Non-fungal victims (Insecta, Poaceae, plant genera) with herbivore/predator attackers
- **~15% of rows**: Fungal victims with viral attackers (mycoviruses, not mycoparasite fungi)
- **~5% of rows**: Plant victims with fungal pathogen attackers (inverted: plant → fungi, not fungi → fungi)
- **<1% of rows**: Actual fungi → mycoparasite fungi relationships (too sparse and noisy)
- **Conclusion**: Mechanism 1 (specific antagonist matches) RARELY fires due to data structure mismatch
...
**Weaknesses**:
1. **GloBI data unusable**: Mechanism 1 essentially non-functional due to inverted relationship direction
   - Kept in code for potential future data improvements
   - Does not affect validity - just returns zero
2. **Scaling factor (×10)**: Half of P1's scaling (×20) - arbitrary
   - **Mitigation**: Percentiles normalize the range
3. **No antagonist efficacy weighting**: *Trichoderma harzianum* ≠ *Coniothyrium minitans* in effectiveness
   - All mycoparasites weighted equally (1.0 per fungus)
   - Could be refined with literature-based efficacy data
