...
**Overall Assessment**: 7/9 components statistically sound, 2 components need refinement.
...
**Source**: `src/Stage_4/guild_scorer_v3.py:649-779` (132 lines - showing core logic)
...
def _compute_n4_csr_conflicts(self, plants_data, n_plants):
    ...
    # ... (similar logic for Competitive vs Ruderal)

    # CONFLICT 4: High-R + High-R
    # ... (low penalty: 0.3 - short-lived annuals)
...
**Formula**:
```
For all plant pairs (i, j):
  base_conflict = f(CSR_i, CSR_j)  # Function of strategy types
...
**Strengths**:
1. **Conflict density normalization**: Dividing by max_pairs makes scores comparable across guild sizes
2. **EIVE-L integration for C-S conflicts**: Shade-adapted S-strategists WANT to be under C-strategists - brilliant ecological insight!
3. **Multi-factor modulation**: Height, form, and light preferences all ecologically justified
4. **Grime theory alignment**: CSR conflict magnitudes match ecological literature

**Weaknesses**:
1. **Arbitrary CSR thresholds**: HIGH_C=60, HIGH_S=60, HIGH_R=50 lack empirical justification
   - **Problem**: Same numeric threshold (60) corresponds to different ecological rarities (Stage 3.3 analysis):
   ...
   - **Implication**: "High C" (90th %ile) ≠ "High S" (75th %ile) in statistical meaning
   - **Stage 3 context**: While percentiles don't improve ecosystem service predictions (Stage 3.4), they ARE appropriate for **conflict detection** where relative position matters
   - **Recommended solution**: **Climate-stratified percentile approach**
     ```python
     # Instead of: if CSR_C > 60 (arbitrary global threshold)
     # Use: if get_csr_percentile(CSR_C, tier, 'C') > 75 (climate-specific top quartile)

     # This ensures "high C" means "top 25% for this climate tier"
     # Accounts for boreal bias (low C baseline) vs tropical bias (high C baseline)
     ```
   - **Reference**: See `Stage_3/3.3_Percentile_vs_Cutoff_Evaluation.md` for detailed analysis
2. **Fixed modulation multipliers**: 0.2×, 0.4×, 0.6× lack empirical justification
   - **No published research** validating these specific multipliers
3. **Conflict types not exhaustive**: What about S-S or S-R pairs?
...
**Statistical Concern**: **Ordinal CSR data treated as continuous** (CSR_C > 60). CSR values from Grime are ordinal scores, not true interval data.

**Recommendation**: **⚠️ Functional but SHOULD adopt percentile approach**
- **Priority action**: Implement climate-stratified CSR percentile classification (like Stage 4.2 framework)
  - Calibrate tier-specific CSR percentiles (already have Köppen tiers from climate filter)
  - Replace `if CSR_C > 60` with `if get_csr_percentile(CSR_C, tier, 'C') > 75`
  - Ensures "high C" means "top quartile for climate" (not arbitrary global threshold)
  - **Precedent**: Stage 3.3 recommended this approach, Stage 4.2 uses it for guild metrics
- Short-term: Document EIVE-L provenance (Dengler et al. 2023) in code comments
- Medium-term: Validate modulation multipliers empirically from guild performance data
- Long-term: Consider continuous CSR interaction functions (after percentile approach tested)
...
2. **Specific vs general weighting**: Specific antagonists (1.0) > general mycoparasites (0.3)
3. **Empirical relationships**: Uses known pathogen-antagonist pairs from literature/databases
4. **Guild-size normalized**: Division by max_pairs

**Weaknesses**:
1. **General mycoparasite weight (0.3)**: Higher than P1's general weight (0.2) - why?
   - **Possible justification**: Fungi are more sessile/persistent than insects
   - **Needs validation**: No citation provided for 0.3 value
2. **Scaling factor (×10)**: Half of P1's scaling (×20) - implies disease control is less important?
   - **This is handled by calibration**: Percentiles normalize the range
3. **No antagonist efficacy weighting**: *Trichoderma* ≠ *Coniothyrium* in effectiveness

**Recommendation**: **Valid as-is**. Weight (0.3) and scaling (×10) are arbitrary but will be normalized by percentile calibration.
...
| 3 | N4 - Growth Compatibility | ⚠️ NEEDS REVIEW | Arbitrary CSR thresholds, ordinal treated as continuous (EIVE-L valid) |
...
**Overall**: **7/9 components statistically sound**, 2 components functional but need refinement.

### Critical Methodological Issues

**Issue 1: Arbitrary CSR Thresholds**
- **N4**: CSR thresholds (60, 60, 50) lack empirical justification
- **Note**: EIVE-L thresholds (3.2, 7.47) are NOT arbitrary - empirically derived from Dengler et al. 2023
- **Impact**: Categorical splits lose information
- **Solution**: Validate CSR thresholds empirically or use continuous functions
...
- **Solution**: These will be absorbed by percentile calibration, but should be documented
...
1. **N4: Implement climate-stratified CSR percentile approach**
   - Calibrate tier-specific CSR percentiles (C, S, R) for all 6 Köppen tiers
   - Replace arbitrary thresholds (60, 60, 50) with percentile-based classification (e.g., >75th percentile)
   - **Reference**: Stage 3.3 analysis + calibration methodology
   - **Benefit**: "High C" will mean "top quartile for climate" (not arbitrary global threshold)
   - **Note**: EIVE-L thresholds already validated (Dengler et al. 2023 - see EIVE_semantic_binning.md)
2. **Code comments**: Add references to EIVE provenance and Stage 3.3 CSR analysis in guild_scorer_v3.py
3. **Test percentile approach**: Compare conflict detection rates (fixed thresholds vs percentile) on calibration dataset
...
The current implementation is **statistically sound for production use**. All 9 components use ecologically justified methods, with 7/9 being rigorous and 2/9 functional but improvable.
...
- EIVE integration for light compatibility

**Key weaknesses**:
- Some arbitrary thresholds (will be normalized by calibration)
- Equal weighting within categories (can be refined)

**Recommendation**: **Proceed with calibration**. The framework is production-ready. Refinements can be made in future iterations based on field validation data.
