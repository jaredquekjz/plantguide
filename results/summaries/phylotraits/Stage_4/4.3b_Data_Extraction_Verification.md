...
## Calibration Verification: 7-Metric Framework (2025-11-05)

### Overview

This section verifies the calibration of the NEW 7-metric framework which consolidates N1/N2 into M1 using phylogenetic distance.
...
**Date**: 2025-11-05
**Script**: `src/Stage_4/calibrate_2stage_koppen.py`

**Stage 1 (2-Plant Pairs)**:
- Guilds per tier: 1,000
- Total guilds: 6,000 (1,000 × 6 tiers)
- Duration: ~15 seconds
- Output: `data/stage4/normalization_params_2plant.json`

**Stage 2 (7-Plant Guilds)**:
- Guilds per tier: 1,000
- Total guilds: 6,000 (1,000 × 6 tiers)
- Duration: ~20 seconds
- Output: `data/stage4/normalization_params_7plant.json`

### Verification Results

#### ✅ VERIFICATION 1: Metric Coverage

**All 7 metrics present in all 6 tiers:**
- m1 (Pest & Pathogen Independence)
- n4 (Growth Compatibility)
- p1 (Insect Control)
- p2 (Disease Control)
- p3 (Beneficial Fungi)
...
**Result**: ✅ 2-plant: All tiers complete | ✅ 7-plant: All tiers complete

#### ✅ VERIFICATION 2: M1 (Pest & Pathogen Independence) Validation

M1 measures phylogenetic distance using mean pairwise eigenvector distances across all 92 phylogenetic eigenvectors.

**Expected**: M1 > 0 for all percentiles (100% coverage, no zeros)

**2-Plant Calibration**:
```
tier_1_tropical:         p01=0.5155, p50=0.6944, p99=0.9628
tier_2_mediterranean:    p01=0.5193, p50=0.6955, p99=0.9775
tier_3_humid_temperate:  p01=0.5127, p50=0.6958, p99=0.9267
tier_4_continental:      p01=0.5055, p50=0.7009, p99=0.9935
tier_5_boreal_polar:     p01=0.4875, p50=0.6917, p99=0.9794
tier_6_arid:             p01=0.5111, p50=0.7127, p99=0.9169
```

**7-Plant Calibration**:
```
tier_1_tropical:         p01=0.5839, p50=0.6841, p99=0.7824
tier_2_mediterranean:    p01=0.6012, p50=0.6946, p99=0.7809
tier_3_humid_temperate:  p01=0.6046, p50=0.6971, p99=0.7806
tier_4_continental:      p01=0.5991, p50=0.6987, p99=0.7814
tier_5_boreal_polar:     p01=0.5715, p50=0.6939, p99=0.7741
tier_6_arid:             p01=0.5874, p50=0.7020, p99=0.7908
```

**Result**: ✅ All M1 values > 0, confirming 100% coverage

**❌ CRITICAL ISSUE DETECTED**: 2-plant and 7-plant guilds have nearly IDENTICAL M1 median values (p50 ≈ 0.69-0.70). This is **ecologically incorrect** - 7-plant guilds should have HIGHER phylogenetic diversity due to dilution effect and cumulative diversity barriers.

**Root Cause**: Using **mean pairwise distance** (MPD) instead of **total phylogenetic diversity**. MPD averages out guild size differences, violating ecological principles.

#### ✅ VERIFICATION 3: N1/N2 Removal Confirmed

**Checked for**: n1, n2 keys in calibration files

**Result**: ✅ No N1/N2 metrics found in either calibration file

**Conclusion**: OLD overlap-counting metrics successfully removed and replaced by M1 phylogenetic distance.

#### ✅ VERIFICATION 4: Bonus Indicator Zero-Inflation

**Expected**: P1, P2, P6 should have high zero rates (not all plants have biocontrol/pollinators)

**Results**:
```
P1 (Insect Control):
  2-plant: 6/6 tiers have p50=0
  7-plant: 6/6 tiers have p50=0
...
**Result**: ✅ Correct zero-inflation pattern confirms sparse GloBI data for these bonus indicators

#### ✅ VERIFICATION 5: Universal vs Bonus Indicator Distribution

**Universal Indicators** (100% coverage):

1. **M1 (Pest & Pathogen Independence)**:
   - 2-plant median: 0.6956
   - 7-plant median: 0.6959
   - **Always > 0** (phylogenetic distance always calculable)

2. **P5 (Structural Diversity)**:
   - 2-plant median: 0.1535
   - 7-plant median: 0.5579
   - **Always > 0** (height data available for all plants)

3. **N4 (Growth Compatibility)**:
   - 2-plant median: 0.0000
   - 7-plant median: 0.0500
   - Can be zero (no CSR conflicts in compatible guilds)

**Bonus Indicators** (dependent on GloBI data):

4. **P3 (Beneficial Fungi)**:
   - 2-plant median: 0.2000
   - 7-plant median: 0.2286
   - Some coverage (~60% of plants have fungi data)

5. **P1, P2, P6**: All zeros at median
   - Very sparse data (<5% coverage)
   - High scores only when data available

**Result**: ✅ Distribution pattern matches expectation

#### ✅ VERIFICATION 6: Calibration Speed Improvement

**OLD Framework (11 metrics with N1/N2)**:
- Duration: ~5-10 minutes per 1000 guilds
- Bottleneck: Counting organism overlaps across entire dataset

**NEW Framework (7 metrics with M1)**:
- Duration: ~20 seconds per 1000 guilds
- Speed: M1 phylogenetic distance calculation is vectorized numpy operation

**Improvement**: **15-30× faster**

**Reason**: M1 calculation uses pre-computed phylogenetic eigenvectors (92 columns per plant). Computing mean pairwise distance is O(n²) in guild size but O(1) in dataset size. N1/N2 required scanning entire organism dataset for each guild.

### Statistical Validity Assessment

#### ❌ M1 (Pest & Pathogen Independence) - **INVALID**

**Current Method**: Mean pairwise phylogenetic distance with exponential transformation

**Formula**:
```python
mean_distance = mean(pairwise_eigenvector_distances)
pest_risk_raw = exp(-3.0 * mean_distance)
m1_norm = normalize_percentile(pest_risk_raw)
```

**Validity**: ❌ **INVALID - REQUIRES REIMPLEMENTATION**

**Problem**: Mean pairwise distance (MPD) is guild-size **independent** by design:
- 2-plant guild: 1 distance → mean = that distance
- 7-plant guild: 21 distances → mean = average of those 21
- Random sampling → similar expected values for both guild sizes

**Ecological Error**: This contradicts the dilution effect and ignores cumulative diversity barriers. A 7-plant guild with high phylogenetic diversity should score MUCH better than a 2-plant pair, but current implementation gives them similar scores.

**Literature Support for Exponential Decay** (still valid):
- Phylopathogen 2013: Pest sharing decays exponentially with phylogenetic distance (k=1.59)
- Gougherty-Davies 2021: Unified phylogenetic signal for pathogens AND herbivores
- Silva-Valderrama 2025: Host phylogeny is PRIMARY predictor of pathogen interaction

**Literature Support for Total Diversity** (not mean):
- Faith's PD (1992): Standard metric in community ecology, measures TOTAL phylogenetic diversity
- Keesing et al. (2006): Dilution effect requires measuring community-level diversity
- Rao's Q: Expected distance between random individuals (equivalent to Faith's PD for equal abundances)

#### ✅ N4 (Growth Compatibility)

**Method**: CSR conflict detection with percentile-calibrated thresholds

**No changes from 9-metric framework** - already statistically valid

**Validity**: ✅ **VALID**

#### ✅ P1-P6 (Bonus Indicators)
...
**Calibration Files**:
- `normalization_params_2plant.json`: 13,030 bytes
- `normalization_params_7plant.json`: 15,220 bytes
- Total percentile points: 546 (6 tiers × 7 metrics × 13 percentiles)

**Coverage**:
```
M1: 100% (phylogenetic distance always available)
N4: 100% (CSR data available)
P1: ~3% (biocontrol data sparse)
P2: ~5% (antagonist fungi sparse)
P3: ~60% (fungal data moderate)
P5: 100% (height data available)
P6: ~15% (pollinator data sparse)
```

---

## Critical Issue: M1 Calculation Error (2025-11-05)

### Issue Summary

**Problem**: M1 (Pest & Pathogen Independence) uses mean pairwise distance (MPD), which produces nearly identical scores for 2-plant and 7-plant guilds.

**Evidence**:
```
2-plant median M1: 0.6956
7-plant median M1: 0.6959
Ratio: 1.00× (essentially identical)
```

**Ecological Error**: A diverse 7-plant guild should have MUCH lower pest/pathogen risk than a 2-plant pair due to:
1. **Dilution effect**: More species dilute pathogen transmission (Keesing et al. 2006)
2. **Cumulative barriers**: More phylogenetic distances = more host-specificity barriers
3. **Resilience**: More species = system continues functioning even if some get pests

### Root Cause Analysis

**Current Implementation (WRONG)**:
```python
# Calculate mean pairwise distance in 92-D eigenvector space
distances = pdist(phylo_eigenvectors, metric='euclidean')
mean_distance = np.mean(distances)  # ← PROBLEM: Averages out guild size
pest_risk_raw = np.exp(-3.0 * mean_distance)
```

**Why This Fails**:
- MPD is a **divergence** metric, not a **richness + divergence** metric
- By definition: mean(2 values) ≈ mean(21 values) when randomly sampled
- Ignores fundamental ecology: larger, diverse guilds are BETTER for pest control

### Solution: Faith's Phylogenetic Diversity (PD)

**Faith's PD** is the gold standard in community ecology for measuring phylogenetic diversity.

**Definition**: Sum of all branch lengths connecting a set of species on a phylogenetic tree.

**Formula**:
```
PD = Σ (branch lengths spanning the species set)
```

**Why Faith's PD is Correct**:
1. **Increases with richness**: More species = more branch lengths = higher PD
2. **Increases with divergence**: Distant species = longer paths = higher PD
3. **Ecologically validated**: Standard metric for 30+ years (Faith 1992)
4. **Matches Rao's Q**: For equal abundances, Rao's Q ≈ Faith's PD

**Comparison of Metrics**:

| Metric | 2-plant median | 7-plant median | Ratio |
|--------|----------------|----------------|-------|
| **Current MPD (eigenvectors)** | 0.1207 | 0.1211 | 1.00× ❌ |
| **Faith's PD (tree)** | 247.47 | 828.01 | **3.35×** ✓ |

**After exponential transformation (k=0.001)**:
- 2-plant pest risk: 0.7808 (HIGH)
- 7-plant pest risk: 0.4369 (LOW)
- 2-plant has **1.79× HIGHER** pest risk ✓

### Implementation Plan

**Step 1: Pre-calculate Faith's PD lookup table**
```bash
# Use R picante (gold standard library)
Rscript src/Stage_4/calculate_faiths_pd.R \
  data/stage1/phlogeny/mixgb_tree_11676_species_20251027.nwk \
  all_plant_combinations.csv \
  faiths_pd_lookup.parquet
```

**Step 2: Update M1 calculation in guild_scorer_v3.py**
```python
# OLD (WRONG):
mean_distance = np.mean(pdist(phylo_matrix))
pest_risk_raw = np.exp(-3.0 * mean_distance)

# NEW (CORRECT):
faiths_pd = lookup_faiths_pd(plant_ids)  # From pre-calculated table
pest_risk_raw = np.exp(-0.001 * faiths_pd)  # Adjusted k for larger scale
```

**Step 3: Update calibrate_2stage_koppen.py** (same change)

**Step 4: Re-run calibrations**
```bash
python src/Stage_4/calibrate_2stage_koppen.py --stage=1 --guilds_per_tier=1000
python src/Stage_4/calibrate_2stage_koppen.py --stage=2 --guilds_per_tier=1000
```

**Step 5: Verify**
- 2-plant and 7-plant scores should now be clearly different
- 7-plant median should be ~3× lower pest risk than 2-plant

### Validation Results

**R picante vs Python ete3** (tested on 100 random guilds):
- Mean absolute difference: < 1e-13
- Max % difference: < 0.0001%
- ✓✓ **Perfect agreement** between implementations

**Guild Size Scaling** (Faith's PD medians):
```
Guild size 2: 255.41
Guild size 3: 366.05
Guild size 5: 608.64
...
**Ratio**: 7-plant / 2-plant = **3.35×** ✓ (ecologically correct)
...
**❌ 2-Plant Calibration**: BLOCKED - Requires Faith's PD recalibration
**❌ 7-Plant Calibration**: BLOCKED - Requires Faith's PD recalibration

**Status**: **IMPLEMENTATION REQUIRED**

Current calibrations are technically complete but ecologically invalid. Faith's PD implementation required before production use.
