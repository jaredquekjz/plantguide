...
## CRITICAL: Exponential Transformation for M1

### Literature Evidence for Exponential Decay

All 4 papers in `papers/mmd/pest_phylogeny/` show pest sharing decays **exponentially** with phylogenetic distance, NOT linearly:
...
**2. Gougherty-Davies 2021 (Table 1)**
```
...
```
Validation: 74.5% correct classification

**Mathematical interpretation:** In real space (not logit), this means:
```
P(pest_sharing) ∝ exp(-k × distance)
```

### Exponential Transformation Applied

**CRITICAL CHANGE from old P4:**

**OLD approach (INCORRECT):**
```python
mean_distance = np.mean(distances)  # Linear
m1_norm = normalize_percentile(mean_distance)  # Wrong! Assumes linear decay
```

**NEW approach (CORRECT):**
```python
mean_distance = np.mean(distances)  # Linear
pest_risk_raw = np.exp(-k * mean_distance)  # Exponential transformation!
m1_norm = normalize_percentile(pest_risk_raw)  # Calibrate transformed value
```

**Decay constant k = 3.0** (calibrated to literature):
...
**Examples:**
```
Monoculture (distance = 0.05): exp(-3.0 × 0.05) = 0.86 (HIGH pest risk)
Moderate    (distance = 0.10): exp(-3.0 × 0.10) = 0.74 (MEDIUM pest risk)
Polyculture (distance = 0.15): exp(-3.0 × 0.15) = 0.64 (LOW pest risk)
```

**After percentile calibration and inversion:**
- Monoculture: 0.86 → ~95th percentile pest_risk → **5th percentile independence** (BAD)
- Polyculture: 0.64 → ~30th percentile pest_risk → **70th percentile independence** (GOOD)

**Aligns with literature:** Congeneric pairs (close) have ~2-3× higher pest sharing than distant pairs.
