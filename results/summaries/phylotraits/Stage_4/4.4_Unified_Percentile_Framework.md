...
**Formula:**
```python
p2_raw = Σ antagonist_mechanisms across all plant pairs

Mechanism 1: Specific pathogen antagonists (weight 1.0)
  Plant B mycoparasites that target Plant A pathogens (via lookup table)

Mechanism 2: General mycoparasites (weight 0.3)
  Plant B has mycoparasites and Plant A has pathogens

p2_normalized = tanh(p2_raw / max_pairs × 10)
```

**Mycoparasite fungi in pathogen control:**
...
- Mechanism 1 uses `pathogen_antagonists.parquet` lookup table (pathogen → antagonist mapping)
- Mechanism 2 provides general disease suppression without specific matching

**Example (4-plant guild):**
- Plant A has botrytis (pathogenic fungus)
- Plant C has trichoderma (mycoparasite that antagonizes botrytis)
- Mechanism 1: 1 match × 1.0 = 1.0
- Mechanism 2: Plant C has mycoparasites and Plant A has pathogens → +1 × 0.3 = 0.3
- p2_raw = 1.3
- max_pairs = 4 × 3 = 12
- p2_normalized = tanh(1.3 / 12 × 10) = tanh(1.08) = 0.795

**Interpretation:** Higher score = stronger pathogen antagonism = good

**Note:** P2 requires pathogen-antagonist lookup table. Set to 0 for initial calibration; fully implemented in guild_scorer_v3.py.
...
**Formula:**
```python
p5_raw = (height_diversity × 0.6) + (form_diversity × 0.4)

where:
  height_diversity = min(height_range / 20.0, 1.0)
    - height_range = max(heights) - min(heights)
    - normalized by 20m (typical full canopy stratification)

  form_diversity = (# unique growth forms) / 5.0
    - 5 = typical max distinct forms (tree, shrub, herb, vine, grass)
```

**Example (5-plant guild):**
- Heights: [0.3m, 2.5m, 8.0m, 15.0m, 0.5m]
  - height_range = 15.0 - 0.3 = 14.7m
  - height_diversity = min(14.7/20.0, 1.0) = 0.735
- Forms: [herb, shrub, tree, tree, herb]
  - unique forms = 3 (herb, shrub, tree)
  - form_diversity = 3/5.0 = 0.6
- p5_raw = (0.735 × 0.6) + (0.6 × 0.4) = 0.441 + 0.240 = 0.681

**Interpretation:** Higher score = better vertical stratification = good
