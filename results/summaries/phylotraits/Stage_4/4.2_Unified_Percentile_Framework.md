# Stage 4.2: Unified Percentile Framework

**Document:** 4.2 Unified Percentile Framework (Complete Implementation Guide)
**Date:** 2025-11-03 (Updated 2025-11-05)
**Purpose:** Complete framework definition + implementation guide for percentile-based guild scoring
**Supersedes:** All previous weighted frameworks (archived in archive/)

---

## ⚠️ IMPORTANT UPDATE (2025-11-05)

**This document has been superseded by Document 4.2c for implementation.**

**What changed:**
- Framework reduced from **9 metrics → 7 metrics**
- **N1 (Pathogen Independence)** and **N2 (Pest Independence)** removed
- **P4 (Phylogenetic Diversity)** repurposed as **M1 (Pathogen & Pest Independence)**
- Added **exponential transformation** (exp(-k × distance)) based on pest phylogeny literature
- Observed pest data moved to qualitative organism profiles (not scoring)

**For implementation details, see:** `4.2c_7_Metric_Framework_Implementation_Plan.md`

**This document (4.2) remains for historical reference only.**
...
**Before (Legacy Weighted Framework - DEPRECATED):**
```
...
**Solution:** Climate-stratified Monte Carlo calibration using Köppen-Geiger zones.
...
**Multi-Assignment:** Plants can belong to multiple tiers if they have ≥5% occurrence in multiple Köppen zones.

**Calibration Strategy:**
- Sample **20,000 guilds per tier** (6 tiers × 20K = 120K total guilds)
...
**User Scoring:**
1. Detect user location → Köppen zone
2. Map Köppen zone → calibration tier
...
guild_score = positive_score - negative_score  # ∈ [-1, +1]
...
# Example:
# n1_raw = 0.82 → percentile = 93.6 (worse than 93.6% of guilds)
# p1_raw = 1.5  → percentile = 99.0 (better than 99.0% of guilds)

# Step 3: Invert negative metrics (HIGH RAW = BAD → LOW DISPLAY)
metrics = {
...
overall_score = mean(metrics.values())  # ∈ [0, 100]
...
def score_guild(self, plant_ids, climate_tier='tier_3_humid_temperate'):
    ...
    # Invert negative metrics (high raw = bad → low display = good)
    metrics = {
...
**Verification:**
1. ✓ No weight constants (grep "0\.35\|0\.20" → 0 matches)
2. ✓ Overall score ∈ [0, 100] (not [-1, +1])
3. ✓ All metrics ∈ [0, 100]
4. ✓ Percentile interpolation working
5. ✓ N5/N6 returned as flags, not scores
...
def generate_explanation(guild_result: Dict) -> Dict:
    ...
    if 'nitrogen' in flags and flags['nitrogen'] == 'Missing':
        recommendations.append('⚠ Add N-fixing species for nitrogen cycling')
    if 'pathogen_independence' in weaknesses:
        recommendations.append('⚠ Consider disease management for shared pathogens')
    if 'phylo_diversity' in weaknesses:
        recommendations.append('⚠ Increase phylogenetic diversity (add different families)')
    if 'structural_diversity' in weaknesses:
        recommendations.append('⚠ Add vertical layering (trees, shrubs, herbs)')
    ...
    return {
        ...
        'flags': {
            'nitrogen': f"{'✓' if flags['nitrogen'] == 'Present' else '⚠'} {flags['nitrogen']}",
            'soil_ph': f"✓ pH range: {flags['soil_ph']}"
        },
...
**Current Output:**
```python
print(f"🎯 FINAL GUILD SCORE: {result['guild_score']:.4f}")
...
**New Output:**
```python
print(f"🎯 OVERALL COMPATIBILITY: {result['overall_score']:.1f}/100")
print(f"   Percentile Ranking: {result['overall_score']:.1f}th percentile")

print(f"\n📊 METRIC BREAKDOWN (all 0-100 scale):")
...
1. `src/Stage_4/guild_scorer_v2.py` → Add header:
```python
"""
...
2. `src/Stage_4/05_compute_guild_compatibility.py` → Add header:
```python
"""
...
3. `src/Stage_4/04_compute_compatibility_matrix.py` → Update docstring:
```python
"""
...
#### Change 5: CSR Percentile Calibration for N4 (ENHANCEMENT)

**Problem:** Current N4 (Growth Compatibility) uses arbitrary fixed thresholds:
```python
HIGH_C = 60
HIGH_S = 60
HIGH_R = 50
...
**Issue:** Same numeric threshold (60) corresponds to different statistical rarities (Stage 3.3 analysis):
- C ≥ 60 → **90th percentile** (very rare, highly competitive)
- S ≥ 60 → **75th percentile** (common in European flora - stress-adapted bias)
- R ≥ 60 → **88th percentile** (rare)

**Solution:** Use global CSR percentile approach for **consistent** threshold interpretation.

**Note:** We do NOT need climate stratification for N4 (unlike the 9 guild metrics). CSR conflicts are within-guild comparisons, so global percentiles provide consistency.

**Implementation Plan:**

**Step 1: Calibrate Global CSR Percentiles**

**Script:** `src/Stage_4/calibrate_csr_percentiles_global.py` (new)
...
def calibrate_csr_percentiles():
    ...
    # Save calibration
    output_path = 'data/stage4/csr_percentile_calibration_global.json'
...
**Expected Output:**
```
C Strategy Percentiles:
  p50 (median): 25.9
  p75 (top quartile): 47.1
  p90 (top decile): 66.7

S Strategy Percentiles:
  p50 (median): 36.3
  p75 (top quartile): 66.4
  p90 (top decile): 88.1

R Strategy Percentiles:
  p50 (median): 26.0
  p75 (top quartile): 49.5
  p90 (top decile): 69.2

COMPARISON: Fixed Thresholds (60, 60, 50) vs Percentiles
========================================================

C ≥ 60:
  Actual percentile: 90th
  Interpretation: Top 10% of plants
  P75 threshold: 47.1 (top quartile)

S ≥ 60:
  Actual percentile: 75th
  Interpretation: Top 25% of plants
  P75 threshold: 66.4 (top quartile)

R ≥ 60:
  Actual percentile: 88th
  Interpretation: Top 12% of plants
  P75 threshold: 49.5 (top quartile)
```

**Step 2: Update N4 to Use CSR Percentiles**
...
def _compute_n4_csr_conflicts(self, plants_data, n_plants):
    ...
    HIGH_C = 60  # Arbitrary global threshold
    HIGH_S = 60
    HIGH_R = 50

    # High-C + High-C conflict
    high_c_plants = plants_data[plants_data['CSR_C'] > HIGH_C]
    # ...
```

**Proposed (percentile-based):**
```python
def __init__(self, ...):
    # ...existing initialization...

    # Load CSR percentile calibration (global, not tier-specific)
    self.csr_calibration_path = Path('data/stage4/csr_percentile_calibration_global.json')
...
def _csr_to_percentile(self, raw_value, strategy):
    ...
    # Linear interpolation (same as _raw_to_percentile)
    if raw_value <= values[0]:
        return 0.0
    if raw_value >= values[-1]:
        return 100.0
...
def _compute_n4_csr_conflicts(self, plants_data, n_plants):
    """
    N4: CSR conflicts with percentile-based thresholds.
    ...
    # Use percentile-based classification (CONSISTENT)
    ...
    # High-strategy classification (CONSISTENT: all use 75th percentile)
    ...
    # (still uses EIVE-L thresholds 3.2, 7.47 - those are empirically validated)
```

**Step 3: Verify Calibration**

Create test script: `src/Stage_4/test_csr_percentile_calibration.py`
...
**Key Decision: Which Percentile Threshold?**

**Options:**
- **P75 (75th percentile)**: "High" = top quartile (25% of plants)
  - More permissive - detects more conflicts
  - Balances sensitivity vs specificity
- **P90 (90th percentile)**: "High" = top decile (10% of plants)
  - More conservative - only detects strong strategists
  - Closer to current C≥60 behavior (90th percentile)

**Recommendation:** Start with **P75 (top quartile)** for consistency across all strategies, then calibrate based on guild performance data.

**Benefits:**
1. **Consistency**: "High C" = "High S" = "High R" = top 25% (all use same percentile)
2. **No climate stratification needed**: CSR conflicts are within-guild, not cross-guild comparisons
3. **Empirically grounded**: Based on actual CSR distribution (11,680 plants)
4. **Flexible**: Can adjust percentile threshold (75 vs 80 vs 90) based on testing

**Trade-offs:**
- **P75 will detect MORE conflicts** than current system (C≥60 is 90th percentile, but we're using 75th)
- **Test on calibration data** to see if this improves guild quality predictions
...
- [ ] Test `_raw_to_percentile()` with known values
  - [ ] Raw = p1 value → percentile = 1
  - [ ] Raw = p50 value → percentile = 50
  - [ ] Raw = p99 value → percentile = 99
  - [ ] Raw < p1 → percentile = 0
  - [ ] Raw > p99 → percentile = 100

- [ ] Test metric inversion
  - [ ] N1 raw high (bad) → pathogen_independence low (correct)
  - [ ] N2 raw high (bad) → pest_independence low (correct)
  - [ ] P1 raw high (good) → insect_control high (correct)

- [ ] Test overall score
  - [ ] All metrics = 100 → overall = 100
  - [ ] All metrics = 0 → overall = 0
  - [ ] Mixed metrics → overall = mean
...
- [ ] Compare 4.3 vs 4.4 rankings
  - [ ] Guild A better than Guild B in 4.3 → should still be better in 4.4
  - [ ] Spearman correlation > 0.9 (rankings preserved)

**Phase 4: Calibration**

2-plant calibration:
- [ ] Run tier-stratified calibration (6 tiers × 20K = 120K guilds)
...
```json
{
  ...
  "flags": {
    "nitrogen": "⚠ Missing",
    "soil_ph": "✓ pH range: 6.0-6.5"
  },
  "recommendations": [
    "⚠ Add N-fixing species for nitrogen cycling",
    "⚠ Consider disease management for shared pathogens",
    "⚠ Increase phylogenetic diversity (different families)",
    "⚠ Add vertical layering (trees, shrubs, herbs)"
  ]
}
```
...
**Version:** 4.2 (Canonical unified percentile framework)
**Last Updated:** 2025-11-04
**Implementation Status:** ✅ Complete (Phase 2 finished)

**Cross-references:**
- Document 4.3: Data Flow and Integration (architectural reference)
- archive/: Deprecated weighted frameworks
