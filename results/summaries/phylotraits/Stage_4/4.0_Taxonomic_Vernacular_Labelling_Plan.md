...
**Objective:** Assign multilingual vernacular names (Phase 1) and gardener-friendly category labels (Phase 2) to all taxa.

**Status:** Refactored 2025-11-16 - Unified directory structure, simplified Phase 1

**Date:** 2025-11-16

**Major Changes:**
- Phase 1: Removed P2/P4 synthetic categories (word frequency) - REAL vernaculars only now
- Phase 2: Moved to unified directory structure
- Created master script to run both phases sequentially
...
**Status:** COMPLETE (Refactored 2025-11-16)

**Coverage:** 67.1% overall (28,002 / 41,738 taxa)
- Plants: 87.9% (10,450 / 11,892 species)
- Animals: 58.8% (17,552 / 29,846 organisms)

**Pipeline location:** `/home/olier/ellenberg/shipley_checks/src/Stage_4/taxonomy/phase1_multilingual/`

**Key Change:** Removed P2/P4 synthetic categories (word frequency derivation). Phase 1 now provides REAL vernacular names only:
...
**Coverage drop explained:** Dropped from 75.2% to 67.1% (-8.1 pp) due to removal of synthetic P2/P4 categories. But quality improved - only real names now.
...
├── phase2_kimi/                 # Phase 2: AI gardener labels
├── legacy/                      # Deprecated methods
├── lib/                         # Shared library functions
└── run_complete_taxonomy_pipeline.sh  # UNIFIED MASTER
```
...
**Time:** Phase 1: ~3 seconds, Phase 2: ~60 minutes
...
**Output:** `data/taxonomy/all_taxa_vernacular_final.parquet` (41,738 taxa, 12 languages)
...
**Location:** `/home/olier/ellenberg/shipley_checks/src/Stage_4/taxonomy/phase2_kimi/`
...
env R_LIBS_USER="/home/olier/ellenberg/.Rlib" \
  /usr/bin/Rscript shipley_checks/src/Stage_4/taxonomy/phase2_kimi/01_aggregate_inat_by_genus.R
```
...
```bash
env R_LIBS_USER="/home/olier/ellenberg/.Rlib" \
  /usr/bin/Rscript shipley_checks/src/Stage_4/taxonomy/phase2_kimi/01b_aggregate_inat_chinese.R
```
...
/home/olier/miniconda3/envs/AI/bin/python \
  shipley_checks/src/Stage_4/taxonomy/phase2_kimi/00_prefilter_animals_only.py
```
...
nohup /home/olier/miniconda3/envs/AI/bin/python -u \
  shipley_checks/src/Stage_4/taxonomy/phase2_kimi/06_kimi_gardener_labels.py \
...
### Run Phase 2 Master Script

**Automated approach** (runs Steps 1-3, provides Step 4 command):
```bash
cd /home/olier/ellenberg/shipley_checks/src/Stage_4/taxonomy
...
# Then follow Step 4 instructions printed by the script
```
