...
**Status:** PIPELINE COMPLETE - 68.0% coverage achieved (9,206/13,416 genera)
...
│  → Threshold: 0.42 (optimized from 0.45)       │
│  → 6,704 categorized (98.0%)                    │
│  → Mean similarity: 0.579                       │
│  → Output: vector_classifications_kalm_chinese  │
└─────────────────────────────────────────────────┘
        ↓
Step 4: Combine with Priority (English > Chinese) ✅
    → 9,206 genera total (68.0% coverage)
    → English: 7,552 | Chinese fills gaps: 1,654
    → Output: vector_classifications_bilingual.parquet
        ↓
Step 5: Threshold Analysis & Sanity Check ✅
    → 60 lowest-scoring samples extracted
    → English @ 0.45: 75-80% accuracy
    → Chinese @ 0.42: 50% accuracy (needs adjustment)
    → Output: reports/sanity_check_low_scores.csv
        ↓
Step 6: Label All Organisms ⏳ PENDING
    → Apply genus→category mapping to 29,846 organisms
    → Output: organisms_categorized_comprehensive.parquet
        ↓
Step 7: Validation & Reports ⏳ PENDING
...
**Overall Coverage:**
- **Total target genera:** 13,416 (10,549 animals + 2,910 plants)
- **Categorized:** 9,206 (68.0%)
- **Uncategorized:** 4,210 (32.0%)
...
| **Chinese** | 6,841 | 0.42 | 6,704 | 98.0% | 0.579 |

**Combined Coverage:**
- English contributes: 7,552 genera (82.0%)
- Chinese fills gaps: 1,654 genera (18.0%)
- Overlap: ~5,000 genera have both languages
- **Coverage gain from Chinese:** +12.2 percentage points

**Quality Assessment:**
- Bilingual mean similarity: 0.624
- Bilingual median: 0.622
- Processing time: ~5 minutes total (English 2.6 min, Chinese 2.1 min)

**Top 10 Categories (Bilingual):**
1. moths: 882 genera (9.6%)
2. caddisflies: 370 (4.0%)
3. hoverflies: 293 (3.2%)
4. tulip trees: 260 (2.8%)
5. leafhoppers: 236 (2.6%)
6. beetles: 222 (2.4%)
7. bees: 202 (2.2%)
8. ground beetles: 202 (2.2%)
9. aphids: 194 (2.1%)
10. assassin bugs: 176 (1.9%)
...
**Sanity Check (60 Lowest-Scoring Samples):**

Manual review of 30 lowest matches per language revealed:
- **English @ 0.45:** ~75-80% accuracy at threshold boundary
  - ✅ Apodemia → butterflies (correct)
  - ✅ Graphium → butterflies (correct)
  - ✅ Peromyscus → mice (correct)
  - ❌ Myrsine → maples (incorrect - is a shrub)

- **Chinese @ 0.42:** ~50% accuracy at threshold boundary
  - ✅ Symbrenthia → butterflies (correct)
  - ✅ Argynnis → butterflies (correct)
  - ❌ Ara → corvids (incorrect - are macaws)
  - ❌ Giraffa → rodents (incorrect - giraffe!)
  - ❌ Actinidia → crabapples (incorrect - kiwifruit)

**Recommendation:** Chinese threshold may need raising to 0.44-0.45 for better quality (would reduce coverage to ~94-95% but improve accuracy).

**Sanity check file:** `reports/sanity_check_low_scores.csv` (60 samples for manual review)
