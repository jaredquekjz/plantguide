# Stage 1 — WorldFlora Matching Workflow
...
conda run -n AI --no-capture-output \
  python src/Stage_1_Data_Extraction/convert_duke_json_to_parquet.py \
...
conda run -n AI --no-capture-output \
  Rscript src/Stage_1_Data_Extraction/worldflora_duke_match.R \
...
cd /home/olier/ellenberg
conda run -n AI --no-capture-output \
  python src/Stage_1_Data_Extraction/convert_eive_csv_to_parquet.py
```
...
R_LIBS_USER='/home/olier/ellenberg/.Rlib' \
conda run -n AI --no-capture-output \
  Rscript src/Stage_1_Data_Extraction/worldflora_eive_match.R \
...
cd /home/olier/ellenberg
conda run -n AI --no-capture-output \
  python src/Stage_1_Data_Extraction/convert_mabberly_csv_to_parquet.py
```
...
R_LIBS_USER='/home/olier/ellenberg/.Rlib' \
conda run -n AI --no-capture-output \
  Rscript src/Stage_1_Data_Extraction/worldflora_mabberly_match.R \
...
cd /home/olier/ellenberg
conda run -n AI --no-capture-output \
  python src/Stage_1_Data_Extraction/convert_austraits_to_parquet.py \
...
R_LIBS_USER='/home/olier/ellenberg/.Rlib' \
conda run -n AI --no-capture-output \
  Rscript src/Stage_1_Data_Extraction/worldflora_austraits_match.R \
...
R_LIBS_USER='/home/olier/ellenberg/.Rlib' \
conda run -n AI --no-capture-output \
  Rscript src/Stage_1_Data_Extraction/worldflora_tryenhanced_match.R \
...
cd /home/olier/ellenberg
R_LIBS_USER='/home/olier/ellenberg/.Rlib' \
  Rscript src/Stage_1_Data_Extraction/extract_try_traits.R
```
...
R_LIBS_USER='/home/olier/ellenberg/.Rlib' \
conda run -n AI --no-capture-output \
  Rscript src/Stage_1_Data_Extraction/worldflora_try_traits_match.R \
...
R_LIBS_USER='/home/olier/ellenberg/.Rlib' \
conda run -n AI --no-capture-output \
  Rscript src/Stage_1_Data_Extraction/worldflora_gbif_match.R \
...
R_LIBS_USER='/home/olier/ellenberg/.Rlib' \
conda run -n AI --no-capture-output \
  Rscript src/Stage_1_Data_Extraction/worldflora_globi_match.R \
...
### 5. Prepare Source/Target WorldFlora Lookups (DuckDB)
...
### 6. Enrich Full GloBI Interactions with WorldFlora IDs
...
- The refreshed WorldFlora installation now reports progress during accepted-ID rewrites (progress interval controlled by `counter` argument).  
- Legacy DuckDB canonicaliser scripts are preserved under `src/Stage_1_Data_Extraction/backup/` for reference.  
- Apply the same workflow to other Stage 1 feeds (EIVE, TRY) by substituting their source paths and maintaining the export → match → merge pattern.
