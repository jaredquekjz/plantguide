...
4. Filter modelling cohorts: save `traits_model_ready_<STAMP>_shortlist.parquet` (11 680 species) and `traits_model_ready_<STAMP>_ge30.parquet` (1 084 species) by joining against `stage1_shortlist_with_gbif_ge30.parquet`.  
5. `python scripts/archive_stage1_trait_logs.py` *(planned)* — package console logs and QA reports under `logs/stage1_modelling_prep/<date>/`.
