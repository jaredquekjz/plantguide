...
### 3.6 Environment-aware BHPMF Roadmap (Planned)

1. **Build per-species environment matrices (means, no quantiles)**  
   - Regenerate `worldclim`, `soilgrids`, and `agroclim` species summaries with `aggregate_stage1_env_summaries.py` whenever GBIF occurrences or rasters change, ensuring fresh `_avg`, `_stddev`, `_min`, `_max` columns (`results/summaries/hybrid_axes/phylotraits/Stage_1/1.5_Environmental_Sampling_Workflows.md`).  
   - Join the full Stage 1 shortlist (≈ 11 680 taxa; `model_data/inputs/trait_imputation_input_shortlist_<STAMP>.csv`) to those summaries, keeping only the `_avg` columns so the feature matrix stays compact.  
   - Persist the wide table as `model_data/inputs/env_features_shortlist_<STAMP>.csv`, include a `species` column mirroring `wfo_accepted_name` (BHPMF expects it), and log the transformation script under `logs/stage1_environment/<STAMP>/`.  
   - Downstream subsets (e.g. ge30 or modelling tiers) can be carved out after the full-shortlist run, so we avoid maintaining parallel imputations.

2. **Augment the BHPMF pass with environment covariates**  
   - Call `phylo_impute_traits_bhpmf.R` with `--add_env_covars=true`, `--env_csv=model_data/inputs/env_features_shortlist_<STAMP>.csv`, and `--env_cols_regex="^(wc|soil|agro).*_avg$"` (tighten the regex if additional columns are kept).  
   - Maintain the existing log/logit trait transforms; the script will z-score each environment column before passing the matrix into `GapFilling`.  
   - Archive outputs under a shortlist-wide diagnostics folder (e.g. `model_data/outputs/diag_bhpmf_shortlist_<STAMP>_env`) so comparisons against the trait-only baseline remain reproducible.

3. **Project to modelling tiers**  
   - After the shortlist run completes, filter the outputs to the 1 084-species modelling roster (or other Stage 2 tiers) and write stamped extracts (e.g. `trait_imputation_bhpmf_ge30_<STAMP>_env.parquet`).  
   - Keep the same provenance flags (`*_imputed_flag`, `bhpmf_env`) so downstream steps can trace whether values came from the environment-aware pass.

4. **Apply environment-aware residual corrections (aHPMF-style)**  
   - For species with observed traits, compute residuals (`observed − bhpmf_env_pred`) and fit ridge regressions per trait using the same environment predictors to capture systematic climate/soil signals.  
   - Use species-stratified cross-validation to tune penalty terms; log RMSE and coverage shifts versus the uncorrected BHPMF run.  
   - Generate adjusted predictions (`bhpmf_env_adj = bhpmf_env_pred + residual_hat`) and propagate combined uncertainty (`sigma_adj = sqrt(sigma_bhpmf^2 + sigma_residual^2)`), writing results to `model_data/outputs/trait_imputation_bhpmf_modelling_shortlist_<STAMP>_env_adjusted.parquet`.

5. **Integrate into the Stage 1 rebuild & modelling master**  
   - Extend `rebuild_stage1_trait_tables.py` to ingest the `_env` and `_env_adjusted` outputs, refresh the trait tables, and update provenance flags (`bhpmf_env`, `bhpmf_env_adjusted`).  
   - Update `model_data/inputs/modelling_master_<STAMP>.parquet` so Stage 2/3 consumers inherit the corrected traits and have access to the mean-based environment matrix for downstream diagnostics.

6. **QA & sign-off**  
   - Re-run coverage deltas, sanity checks, and trait distribution plots; store the findings in `logs/stage1_modelling_prep/<STAMP>/env_bhpmf_qa.md`.  
   - Compare Stage 2 CLM fits using the adjusted traits versus the current baseline; document the effect sizes before release.  
   - Coordinate with XGBoost owners to ensure schema alignment and to decide whether quantile-heavy features remain necessary once mean-based covariates power the imputation.
