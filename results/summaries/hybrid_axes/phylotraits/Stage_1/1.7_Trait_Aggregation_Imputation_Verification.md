...
1. **Build per-species environment matrices (means, no quantiles)**  
   ...
   - Join the ge30 roster (`model_data/inputs/traits_model_ready_<STAMP>_ge30_phylo.parquet`) to those summaries and retain only the `_avg` (and, if desired, `_stddev`) columns to avoid quantile proliferation.  
   - Persist the wide table as `model_data/inputs/env_features_ge30_<STAMP>.csv` with one row per `wfo_accepted_name`, keeping `Genus`/`Family` for downstream merges and logging the transformation script under `logs/stage1_environment/<STAMP>/`.

2. **Augment the BHPMF pass with environment covariates**  
   - Call `phylo_impute_traits_bhpmf.R` with `--add_env_covars=true`, `--env_csv=model_data/inputs/env_features_ge30_<STAMP>.csv`, and `--env_cols_regex="^(wc|soil|agro).*_(avg|stddev)$"` (tighten the regex once the final column set is confirmed).  
   ...
   - Archive outputs under a new diagnostics folder (e.g. `model_data/outputs/diag_bhpmf_modelling_shortlist_<STAMP>_env`) to keep comparisons against the trait-only baseline reproducible.

3. **Apply environment-aware residual corrections (aHPMF-style)**  
...
4. **Integrate into the Stage 1 rebuild & modelling master**  
