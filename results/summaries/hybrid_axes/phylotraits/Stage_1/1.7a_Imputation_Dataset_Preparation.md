...
Direct assembly from canonical sources (no Perm 8 dependency):

```
BHPMF Canonical (172 cols)
├─ Identifiers (4): wfo_taxon_id, wfo_accepted_name, Genus, Family
├─ Target traits (6): raw scale
├─ Log transforms (6): pre-computed auxiliary features
└─ Environmental q50 (156)

Constraint: Numeric matrix only (no categorical, no eigenvectors)
```
...
- **BHPMF:** Uses raw traits but masks them cell-wise during CV (different methodology).
...
**Environmental features (156):** Pre-merged into canonical input
...
### 5.5 Key Differences vs XGBoost

**Feature compatibility:**
- ✓ Both use same trait base (canonical SLA)
- ✓ Both use same environmental features (156 q50)
- ✓ Both use same log transforms (6)
- ✗ BHPMF cannot use categorical features (TRY traits)
- ✗ BHPMF cannot use phylogenetic eigenvectors
- ✓ BHPMF requires genus/family hierarchy (not in XGBoost)

**Methodological differences:**
- XGBoost: Log transforms as explicit predictors
- BHPMF: In-place transformation (raw → log → z-standardize → BHPMF → back-transform)
...
| `create_bhpmf_canonical_input.py` | Build BHPMF canonical dataset | `scripts/` |
