...
**Problem:** 46.4% of species (5,419 / 11,680) have NO observed EIVE values → cannot use cross-axis EIVE features

**Solution:** Train two sets of models for each axis

#### Model Set 1: Full Models (WITH cross-axis EIVE)
- **Training:** ~6,200 species with observed EIVE (Tier 2 CV set)
- **Features:** All predictors including cross-axis EIVE (e.g., EIVEres-M for predicting L)
- **Use for:** 337 species with partial EIVE (1-4 axes observed, some cross-axis features available)
- **Status:** ✓ COMPLETED (same as Tier 2 production CV)
- **Performance:** R² 0.506-0.823

#### Model Set 2: No-EIVE Models (WITHOUT cross-axis EIVE)
- **Training:** Same ~6,200 species (Tier 2 CV set)
...
- **Status:** ✓ COMPLETED (2025-10-29)
- **Performance:** R² 0.448-0.807 (avg -7.5% vs full models)
- **Runtime:** ~6 minutes (all 5 axes)

**Performance Comparison:**
| Axis | Full R² | No-EIVE R² | Δ R² | % Drop | Notes |
|------|---------|------------|------|--------|-------|
| T | 0.823 | 0.807 | -0.016 | -1.9% | Minimal loss (climate-driven) |
| M | 0.704 | 0.662 | -0.042 | -6.0% | Lost EIVEres-N (#2 predictor) |
| L | 0.664 | 0.615 | -0.049 | -7.4% | Lost EIVEres-M (#1 predictor) |
| R | 0.506 | 0.448 | -0.058 | -11.5% | Lost EIVEres-N (#2 predictor) |
| N | 0.694 | 0.618 | -0.076 | -11.0% | Lost EIVEres-M (#2 predictor) |

**Key Insight:** No-EIVE models lose 6-16% of cross-axis EIVE importance but remain scientifically valid (R² 0.448-0.807) for species lacking all observed EIVE.
...
### 3.4 Imputation Strategy (Pending)

**Apply appropriate model based on EIVE missingness:**

| Species EIVE Pattern | Count | Model Type | Rationale |
|---------------------|-------|------------|-----------|
| Complete (all 5 axes) | 5,924 (50.7%) | None needed | Already observed |
| Partial (1-4 axes) | 337 (2.9%) | Full models | Some cross-axis EIVE available |
| None (0 axes) | 5,419 (46.4%) | No-EIVE models | No cross-axis EIVE available |

**Next step:** Implement hybrid imputation script `src/Stage_2/impute_missing_eive_hybrid.py`
...
|------|---------|------------|------------------|---------------------|
| T | 0.823 | 0.807 | -1.9% | Minimal (climate-dominated) |
| M | 0.704 | 0.662 | -6.0% | Moderate (lost EIVEres-N) |
| L | 0.664 | 0.615 | -7.4% | Moderate (lost EIVEres-M top predictor) |
| R | 0.506 | 0.448 | -11.5% | Larger (lost EIVEres-N #2) |
| N | 0.694 | 0.618 | -11.0% | Largest (lost EIVEres-M #2) |

**Average drop:** 7.5% R² without cross-axis EIVE coupling
...
| Partial EIVE (1-4 axes) | 337 (2.9%) | Full models | `impute_missing_eive_hybrid.py` |
| No EIVE (0 axes) | 5,419 (46.4%) | No-EIVE models | `impute_missing_eive_hybrid.py` |
