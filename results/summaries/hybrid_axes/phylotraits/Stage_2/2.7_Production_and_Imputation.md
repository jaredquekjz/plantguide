...
2. Remove ALL EIVE-related predictors (10 features total):
   - Direct EIVE: EIVEres-L, EIVEres-T, EIVEres-M, EIVEres-N, EIVEres-R (5 features)
   - EIVE-based phylo: p_phylo_L, p_phylo_T, p_phylo_M, p_phylo_N, p_phylo_R (5 features)
3. Keep: traits, phylo eigenvectors (non-EIVE), environment, categorical
4. Result: ~6,200 species × 732 features (9 EIVE-related columns removed per axis)
...
**Input (per axis):** No-EIVE feature tables from 3.4 (excludes 10 EIVE-related features)

**Training data:** Same species as full models (~6,200 per axis with observed EIVE)

**Excluded features (10 total):**
- Direct EIVE: EIVEres-L, T, M, N, R (5 features)
- EIVE-based phylo: p_phylo_L, T, M, N, R (5 features)
...
**Note:** These models correctly exclude ALL 10 EIVE-related features (5 direct + 5 phylo). Average 8.8% performance drop vs full models is expected and acceptable for imputing species without EIVE.
...
**Rationale:**
- NO-EIVE models exclude all 10 EIVE-related features (5 direct EIVE + 5 EIVE-based phylo)
